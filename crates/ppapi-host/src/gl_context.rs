...
// via the thread_local CURRENT_GL_RESOURCE tracking.
...
impl OffscreenGlContext {
    ...
    pub fn readback_bgra(&self, output: &mut Vec<u8>) {
        ...
        // Bind the correct read target: our offscreen FBO, or FBO 0 (the
        // pbuffer default framebuffer) on the non-FBO path.
        unsafe {
        ...
        // Games typically don't manage the framebuffer alpha channel, leaving
        // it at 0 (the clear value), which would make the frame transparent.
...
impl Drop for OffscreenGlContext {
    fn drop(&mut self) {
        ...
        // glutin handles eglDestroySurface/eglDestroyContext on drop.
    }
}
